...
const Pending = () => {
  ...
  const searchVal = sessionStorage.getItem("searchValPend");
  ...
  useEffect(() => {
    ...
    let data = pendingData.filter((row) => {
  ...
  const handleExportData = () => {
    ...
    if (exportData.length > 0) {
      ...
      XLSX.writeFile(
        workbook,
        `Export_Pending_Sites_${new Date().toISOString()}.xlsx`
      );
    } else {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (searchRef.current && !searchRef.current.contains(e.target)) {
        if (search) {
          sessionStorage.setItem("searchValPend", search);
        }
      }
    };
