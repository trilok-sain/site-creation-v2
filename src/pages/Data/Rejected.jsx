...
const Rejected = () => {
  ...
  const searchVal = sessionStorage.getItem("searchValRej");
  ...
  useEffect(() => {
    ...
    const data = tableData.filter((row) => {
      ...
      //     row.status?.includes("REJECTED") ||
      //     row.adminStatus?.includes("REJECTED") ||
      //     // row.superdminStatus?.includes("REJECTED")
      //   );
      // } else if (roleId == 3) {
      //   return (
      //     row.status?.includes("REJECTED") ||
  ...
  useEffect(() => {
    ...
    let data = rejectedData.filter((row) => {
      ...
      return (
        ...
        optionName?.includes(searchString) ||
  ...
  const handleExportData = () => {
    ...
    if (exportData.length > 0) {
      ...
      XLSX.writeFile(
        workbook,
        `Export_Rejected_Sites_${new Date().toISOString()}.xlsx`
      );
    } else {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (searchRef.current && !searchRef.current.contains(e.target)) {
        if (search) {
          sessionStorage.setItem("searchValRej", search);
        }
      }
    };
  ...
  const handleFilterTable = () => {
    if (fromDate && toDate) {
      const filtered = rejectedData.filter((item) => {
        return (
          fromDate &&
          toDate &&
