...
const All = () => {
  ...
  // const exportFilterData = exportData.filter(obj1 => filteredData.some(obj2 => obj1?.siteID === obj2?.siteID))
  // console.log("exportfilterdata", exportFilterData)
  ...
  const handleExportData = () => {
    ...
    XLSX.writeFile(
      workbook,
      `Export_All_Sites_${new Date().toISOString()}.xlsx`
    );
  };
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (searchRef.current && !searchRef.current.contains(e.target)) {
        if (search) {
          sessionStorage.setItem("searchVal", search);
        }
      }
    };
    ...
    if (fromDate && toDate) {
      ...
      const filtered = tableData.filter((item) => {
        return (
          fromDate &&
          toDate &&
          new Date(fromDate) <= new Date(item?.createdOn) &&
          new Date(toDate) >= new Date(item?.createdOn)
        );
      });
