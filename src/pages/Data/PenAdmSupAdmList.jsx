...
const PenAdmSupAdmList = () => {
  ...
  const [penAdmSupAdmList, setPenAdmSupAdmList] = useState([]);
  ...
  const searchVal = sessionStorage.getItem("searchValPenAdmSup");
  ...
  useEffect(() => {
    ...
    let data = penAdmSupAdmList.filter((row) => {
  ...
  const handleExportData = () => {
    ...
    if (exportData.length > 0) {
      ...
      XLSX.writeFile(
        workbook,
        `Export_PenAdmSupAdminList_Sites_${new Date().toISOString()}.xlsx`
      );
    } else {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (searchRef.current && !searchRef.current.contains(e.target)) {
        if (search) {
          sessionStorage.setItem("searchValPenAdmSup", search);
        }
      }
    };
  ...
  const handleFilterTable = () => {
    if (fromDate && toDate) {
      const filtered = penAdmSupAdmList.filter((item) => {
        return (
          fromDate &&
          toDate &&
...
export default PenAdmSupAdmList;
